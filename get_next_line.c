...
/*   Updated: 2022/12/29 03:50:14 by snaggara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static int	nb_line;
	static struct s_static	*data_static;
...
int	get_length_line_buffer(struct s_static *data_static)
{
	...
	i = 0;
	while (cursor < data_static->nb_octet &&
...
int	main()
{
	...
	if (!fd)
		return (0);
	//printf("%s\n",get_next_line(fd));
	t_static.cursor_beg = 8;
	if (t_static.cursor_beg == 7)
		return (1);
